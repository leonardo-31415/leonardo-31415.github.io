...
switch($data->action) {
	case 'create':
		$sql = "INSERT INTO annotations_lamina (".implode(', ', $keys).") " .
	...
	case 'delete':
		$sql = "DELETE FROM annotations_lamina WHERE id = :id";
		$q = $pdo->prepare($sql);
	...
	case 'update':
		$sql = "UPDATE annotations_lamina set";
		$tmp = [];
		foreach($vars as $key => $value) {
			if($key != 'id')
				$tmp[] = "`$key` = :$key ";
...
function export($pdo) {
		$sql = "select * from annotations_lamina";
		$stm = $pdo->query($sql);
		if(!$stm) {
