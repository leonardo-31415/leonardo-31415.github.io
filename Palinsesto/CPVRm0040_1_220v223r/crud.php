...
switch($data->action) {
	case 'create':
		$sql = "INSERT INTO annotations_moneta2 (".implode(', ', $keys).") " .
	...
	case 'delete':
		$sql = "DELETE FROM annotations_moneta2 WHERE id = :id";
		$q = $pdo->prepare($sql);
	...
	case 'update':
		$sql = "UPDATE annotations_moneta2 set";
		$tmp = [];
		foreach($vars as $key => $value) {
			if($key != 'id')
				$tmp[] = "`$key` = :$key ";
...
function export($pdo) {
		$sql = "select * from annotations_moneta2";
		$stm = $pdo->query($sql);
		if(!$stm) {
