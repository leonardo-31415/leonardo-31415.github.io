...
switch($data->action) {
	case 'create':
		$sql = "INSERT INTO annotations_tessuto (".implode(', ', $keys).") " .
	...
	case 'delete':
		$sql = "DELETE FROM annotations_tessuto WHERE id = :id";
		$q = $pdo->prepare($sql);
	...
	case 'update':
		$sql = "UPDATE annotations_tessuto set";
		$tmp = [];
		foreach($vars as $key => $value) {
			if($key != 'id')
				$tmp[] = "`$key` = :$key ";
...
function export($pdo) {
		$sql = "select * from annotations_tessuto";
		$stm = $pdo->query($sql);
		if(!$stm) {
