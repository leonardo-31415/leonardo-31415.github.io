...
switch($data->action) {
	case 'create':
		$sql = "INSERT INTO annotations_$dataset (".implode(', ', $keys).") " .
	...
	case 'delete':
		$sql = "DELETE FROM annotations_$dataset WHERE id = :id";
		$q = $pdo->prepare($sql);
	...
	case 'update':
		$sql = "UPDATE annotations_$dataset set";
		$tmp = [];
		foreach($vars as $key => $value) {
			if($key != 'id')
				$tmp[] = "`$key` = :$key ";
...
function export($pdo) {
		$sql = "select * from annotations_$dataset";
		$stm = $pdo->query($sql);
		if(!$stm) {
